...
import { EventFormData, eventFormSchema } from "@/utils/eventForm.schema";
import {
  Form,
  FormControl,
  FormField,
...
interface EventFormProps {
  mode: "create" | "edit";
  prefetchData?: Preloaded<typeof api.events.getEventById>;
}

function EventForm({ mode, prefetchData }: EventFormProps) {
  const { user } = useUser();
  ...
  const initialData = prefetchData
    ? usePreloadedQuery(prefetchData)
    : undefined;
  const currentImageUrl = useStorageUrl(
    initialData?.imageStorageId as Id<"_storage">
  );
