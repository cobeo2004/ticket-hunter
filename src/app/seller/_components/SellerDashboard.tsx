...
import { createStripeConnectWithCustomer } from "@/actions/createStripeConnectWithCustomer";
...
function SellerDashboard() {
  ...
  const { user } = useUser();
  const stripeConnectId = useQuery(api.users.getStripeConnectIdFromUser, {
    userId: user?.id ?? "",
  });

  useEffect(() => {
    if (stripeConnectId) fetchAccountStatus();
  }, [stripeConnectId]);
            ...
            <div className="text-center py-8">
              ...
              <button
                onClick={async () => {
                  ...
                  setError(false);
                  try {
                    await createStripeConnectWithCustomer();
